...
2. Choose 3–5 **Separation colors**, leave **Remove background**, **Preserve dark details**, **Merge lighting / shadow shades** and **One material silhouette** enabled for a single-color artwork, then click **Separate into layers**. The silhouette option makes the non-dark foreground one editable material; eyes and a mouth can remain separate dark parts. Turn it off when the source really has multiple material colors. Transparent images use their alpha mask. Turn off background removal to use a carefully drawn foreground mask as-is.
3. Review the simplified artwork. **Layers** lists material layers and connected dark details, with lighting bands merged when that option is enabled. Each row is one proposed object part and its swatch is the proposed thread color; the algorithm does not automatically know names such as eye, ear, or mouth. Double-click a part's name to rename it.
