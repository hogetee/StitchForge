...
2. Choose 3–5 **Separation colors**, leave **Remove background**, **Preserve dark details** and **Merge lighting / shadow shades** enabled, and click **Separate into layers**. The merge option prevents a highlight or cast shadow from becoming a new thread/material layer. Enable **One material silhouette** when this artwork should be one non-dark piece; dark details such as eyes and a mouth can remain editable parts. Transparent images use their alpha mask. Turn off background removal to use a carefully drawn foreground mask as-is.
