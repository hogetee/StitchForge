...
- Tests: 47 public tests plus one configured private-image test pass (48 total). Tests cover background IoU, eye/mouth recall, deterministic proposals, disjoint masks, common coordinates, layer order/visibility, painting ownership, project roundtrip/rejection, editable EMB project export, direction overrides, fill-row progress/cancellation, and a complete Qt layer edit/save/reopen/export workflow. Native macOS project startup succeeds.
...
The app's **Export EMB (editable)** writes a Threadform project package with an `.emb` extension so source, masks, colors, order and settings can be reopened and edited in Threadform. Native Wilcom EMB output remains outside this open-source implementation.
